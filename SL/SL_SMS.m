...
/*- (SMS *)createSLMessageWithReduced:(BOOL)reduced zones:(NSArray *)zones {
    ...
    // Denna sträng är nu också obsolete, eftersom den endast var för den korta perioden innan 1 feb.
    ...
    NSString *returnString = [NSString stringWithFormat:@"%@-%@ %@:%@ 651\n\nEEOOXXAOOE\nEXXEOEEEAA\nEXAOEEEEXO\nEEEEEEEEEE\n\nSL biljett giltig till %@:%@ %d-%@-%@\n%@ %d kr ink 6%% \nmoms\n%@\nm.sl.se", priceText, zonesString, validHourStr, validMinuteStr,
                              validHourStr, validMinuteStr, validYear, validMonthStr, validDayStr,
                              priceText2, price, checksum];
    
    [sms setMessage:returnString];
    [sms setSender:[NSString stringWithFormat:@"SL651"]];
    ...
    [sms setLinks:[NSArray arrayWithObjects:[NSString stringWithFormat:@"%@ %d-%@-%@", validMinuteStr, validYear, validMonthStr, validDayStr], @"m.sl.se", nil]];
    
    return sms;
}*/

- (NSString *)generateRandomAEOX {
    NSMutableString *str = [[NSMutableString alloc] init];
    
    NSArray *letters = [NSArray arrayWithObjects:@"A", @"E", @"O", @"X", nil];
    
    for(int i = 0; i < 9; i++) {
        [str appendFormat:@"%@", [letters objectAtIndex:(arc4random() % letters.count)]];
    }
    
    return str;
}

// Ny version fr.om. 1 februari 2013
// Från telenummer SLXXX (där XXX är randomint1, randomint2, randomint3)
- (SMS *)createSLMessageWithReduced:(BOOL)reduced zones:(NSArray *)zones {
    ...
    // 1. Used as char, in phone number #1
    NSInteger randomint1 = arc4random() % 6;
    
    // 2. Used in phonenumber #2
    NSInteger randomint2 = arc4random() % 9;
    
    // 3. Used in phonenumber #3
    NSInteger randomint3 = arc4random() % 9;
    ...
    NSMutableString *randomIntegers = [[NSMutableString alloc] init];
    for(int i = 0; i < 12; i++)
        [randomIntegers appendFormat:@"%d", (arc4random() % 9)];
    
    NSString *checksum = [NSString stringWithFormat:@"%@%d%d%d", randomIntegers, randomint1, randomint2, randomint3];
    
    // priceText-zones validityHour:validityMinute randomint1randomint2randomint3
    // EOAOAOOOEOE?
    // EOXOXOEA?
    // EOOXOXOXOX?
    // EEEEEEEEEEE?
    ...
    NSString *middleChecksum = [NSString stringWithFormat:@"E%@\n"
                                "E%@\n"
                                "E%@\n"
                                "EEEEEEEEEE", [self generateRandomAEOX], [self generateRandomAEOX], [self generateRandomAEOX]];
    
    NSString *zonesString = [[zones valueForKey:@"description"] componentsJoinedByString:@""];
    
    NSString *returnString = [NSString stringWithFormat:@"%@-%@ %@:%@ %d%d%d\n\n%@\n\nSL biljett giltig till %@:%@ %d-%@-%@\n%@ %d kr ink 6%% \nmoms\n%@\nm.sl.se", priceText, zonesString, validHourStr, validMinuteStr, randomint1, randomint2, randomint3, middleChecksum,
                              validHourStr, validMinuteStr, validYear, validMonthStr, validDayStr,
                              priceText2, price, checksum];
    
    [sms setMessage:returnString];
    [sms setSender:[NSString stringWithFormat:@"SL%d%d%d", randomint1, randomint2, randomint3]];
    [sms setDate:[NSDate date]];
    //[sms setLinks:[NSArray arrayWithObjects:[NSString stringWithFormat:@"%@%d%@%@", validMinuteStr, validYear, validMonthStr, validDayStr], @"http://mobil.sl.se", nil]];
    [sms setLinks:[NSArray arrayWithObjects:[NSString stringWithFormat:@"%@ %d-%@-%@", validMinuteStr, validYear, validMonthStr, validDayStr], @"m.sl.se", /*@"076-7201010", @"http://sl.se/sms", @"http://sl.se",*/ nil]];
